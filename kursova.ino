...
const String labels[] = {"Lv", "Zh", "Room"};
// Using POSIX timezone string for Ukraine (EET/EEST with automatic DST transition)
...
// ── State Variables ──────────────────────────────────────────────────────
float cachedTemp[3] = {0, 0, 0};
int   cachedHum[3]  = {0, 0, 0};
bool  dataReady[3]  = {false, false, false};
int           currentCity       = 0;
...
String isoTimestamp() {
  ...
  time_t g_time = mktime(g);
  time_t l_time = mktime(s);
  long offset_seconds = l_time - g_time;
  ...
  snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02d%+03d:00",
    ...
    s->tm_hour, s->tm_min, s->tm_sec, offset_hours);
  return String(buf);
}

String isoTimestampDisplay() {
  time_t now = time(nullptr);
  struct tm* s = localtime(&now);
  char buf[20];
...
bool sendToSupabase(int idx) {
  if (!dataReady[idx]) return false;

  WiFiClientSecure client;
  client.setInsecure();

  HTTPClient https;
  String url = SUPABASE_URL + "/rest/v1/weather_logs";
  ...
  doc["temp_c"] = cachedTemp[idx];

  if (idx == 2) {
    doc["humidity_pct"] = nullptr;
  } else {
    doc["humidity_pct"] = cachedHum[idx];
  }
  ...
  if (code == 201) {
    Serial.printf("[DB] OK: %s %.1fC\n", labels[idx].c_str(), cachedTemp[idx]);
    https.end();
    return true;
  } else {
    Serial.printf("[DB] Error %d for %s\n", code, labels[idx].c_str());
...
void logWeather() {
  ...
  if (!f) { Serial.println("[LOG] Open error");
    return; }

  String ts = isoTimestampDisplay();
  for (int i = 0; i < 3; i++) {
...
String buildIndexPage() {
  ...
  String html = 
    ...
    "</style></head><body>"
    "<h2>&#127781; Meteo Logs</h2>"
    "<p class='sub'>" + isoTimestampDisplay() + " &nbsp;|&nbsp; " + WiFi.localIP().toString() + "</p>"
  ...
  for (int i = 0; i < LOG_FILES; i++) {
    ...
    html += "</td><td>";
    if (i == logFileIndex)
      html += "<span class='cur'>&#9679; active (" + String(logEntries) + "/" + String(MAX_ENTRIES) + ")</span>";
    else if (exists)
      html += "archive";
    else
...
void handleLogFile(int idx) {
  ...
  File f = LittleFS.open(path, "r");
  server.sendHeader("Content-Disposition", "attachment; filename=\"log" + String(idx) + ".csv\"");
